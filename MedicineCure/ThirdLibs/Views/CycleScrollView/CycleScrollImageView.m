...
- (id)initWithFrame:(CGRect)frame cycleDirection:(CycleDirection)direction pictures:(NSArray *)imagesUrlArr defaultImg:(UIImage *)defaultImg;
{
    ...
    if(self)
    {
        ...
        totalPage               = (int)imagesUrlArr.count;
            /// 显示的是图片数组里的第一张图片
        curPage                 = 1;
...
{
    pageControl.hidden = !isShowPageControl;
...
- (void)refreshScrollView
{
    ...
    {
        NSString *imgUrl = [curImages objectAtIndexSafe:i];
        ...
        else
        {
            ...
            [imageView sd_setImageWithURL:[NSURL URLWithString:imgUrl] placeholderImage:self.defaultImg completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
- (NSArray *)getDisplayImagesWithCurpage:(int)page 
{
    ...
    [curImages addObject:[self.imagesUrlArr objectAtIndexSafe:pre-1]];
    [curImages addObject:[self.imagesUrlArr objectAtIndexSafe:curPage-1]];
    [curImages addObject:[self.imagesUrlArr objectAtIndexSafe:last-1]];
