...
-(void)addToWindow
{
    ...
    else [[[window subviews] objectAtIndexSafe:0] addSubview:self.view];
...
-(void)removeAlertFromView
{
    ...
    {
        MBAlertView *alert = [displayQueue objectAtIndexSafe:0];
        currentAlert = alert;
...
-(void)didSelectButton:(MBAlertViewButton*)button
{
    if(button.tag >= _items.count)
        return;
    MBAlertViewItem *item = [_items objectAtIndexSafe:button.tag];
    if(!item)
        return;
...
-(void)didTapOutsideOfButtons:(UITapGestureRecognizer *)recognizer
{
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        if(_buttons.count == 1)
        {
            MBAlertViewButton *alertButton = [_buttons objectAtIndexSafe:0];
...
-(UIButton*)bodyLabelButton
{
    ...
    CGSize size = getStringSize(_bodyText, self.bodyFont, [self labelConstraint]);
