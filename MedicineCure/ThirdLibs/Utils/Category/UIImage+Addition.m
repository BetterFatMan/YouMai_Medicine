...
+ (UIImage *)contentImageSize:(NSString *)imageName imageSize:(CGSize)size compressionQuality:(CGFloat)quality
{
    ...
    UIImage *img = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    NSData *adata = UIImageJPEGRepresentation(img, quality);
    if (!adata)
    {
        adata = UIImagePNGRepresentation(tempImage);
    }
