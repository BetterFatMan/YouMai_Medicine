...
-(void)startLoadingView
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(tempDateComponents)
        {
            ...
            self.currentDateComponents = [currentLKCalendar components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit fromDate:[NSDate date]];
        }
    });
}
-(void)reloadViewDate
...
-(void)setCurrentDateComponents:(NSDateComponents *)com animation:(BOOL)animation
{
    ...
    {
        NSInteger monthdiff = monthDiffWithDateComponents(_currentDateComponents,com);
        if(monthdiff==0)
            return;
...
-(id)moveToDate:(NSDate *)date
{
    ...
    NSInteger  index = [monthView.dates indexOfObject:date];
